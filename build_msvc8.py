...
class MSVC(Backend):
	...
	def qtPath(self, path):
		if self.switches.dynamic():
			return self.build.absPath('./Extern/Win/Qt/4.5.2/vs8-dll' + path)
		return self.build.absPath('./Extern/Win/Qt/4.5.2/vs8-lib' + path)
	
	def qtBinPath(self):
		return self.build.absPath('./Extern/Win/Qt/4.5.2/bin')

	def qtLibPath(self, name):
		if self.switches.dynamic() and name != 'qtmain':
			if self.switches.debug():
