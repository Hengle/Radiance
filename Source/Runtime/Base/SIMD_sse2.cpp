...
void SkinVerts3B2T( // Becky, this one isn't finished / tested.
	float *outVerts, 
	const float *bones, 
	const float *vertices,
	const U16 *boneIndices,
	...
	for (int i = 0; i < numVerts; ++i) {
		...
		__asm {
			...
			mulps    xmm1, [edx+0x10] // t3 yyyy * row1
			addps    xmm4, xmm5 // t1 + t2 xxxx yyyy
			movaps   xmm7, xmm2 // t3 zzww
			movlhps  xmm2, xmm2 // t3 zzzz
			movhlps  xmm7, xmm7 // t3 wwww
			mulps    xmm2, [edx+0x20] // t3 zzzz * row2
			
			// t4
			mov      edx, bone0
			// stall xmm1 (1)
			addps    xmm1, xmm0 // t3 xxxx + yyyy
			movaps   xmm0, [esi+0x100] // t4 xyzw
			movaps   xmm4, [esi+0x110] // t4 xyzw
			movaps   xmm3, xmm0 // t4 xyzw
			unpcklps xmm0, xmm0 // t4 xxyy
			addps    xmm6, xmm1 // t1 + t2 + t3 xxxx yyyy
			unpckhps xmm3, xmm3 // t4 zzww
			movaps   xmm1, xmm0 // t4 xxyy
			movlhps  xmm0, xmm0 // t4 xxxx
			addps    xmm6, xmm2 // t1 + t2 + t3
			movhlps  xmm1, xmm1 // t4 yyyy
			mulps    xmm0, [edx] // t4 xxxx * row0
			mulps    xmm1, [edx+0x10] // t4 yyyy * row1
			shufps   xmm6, xmm6, 93h // t123 wxyz
			movlhps  xmm3, xmm3 // t4 zzzz
			movss    xmm6, xmm7 // t123 (w)xyz
			
			mulps    xmm3, [edx+0x20] // t4 zzzz * row2
			shufps   xmm6, xmm6, 39h // t123 xyzw

			// t5
			mov      edx, bone1
			addps    xmm1, xmm0 // t4 xxxx + yyyy
			movaps [edi+0x20], xmm6 // [store] tangent
			movaps   xmm6, xmm4 // t5 xyzw
			unpcklps xmm4, xmm4 // t5 xxyy
			unpckhps xmm6, xmm6 // t5 zzww
			addps    xmm3, xmm1 // t4 xxxx + yyyy + zzzz
			movaps   xmm5, xmm4 // t5 xxyy
			movlhps  xmm4, xmm4 // t5 xxxx
			movhlps  xmm5, xmm5 // t5 yyyy
			mulps    xmm4, [edx] // t5 xxxx * row0
			mulps    xmm5, [edx+0x10] // t5 yyyy * row1
			movlhps  xmm6, xmm6 // t5 zzzz
			
			// t3
			movaps   xmm0, [esi+0x120] // t6 xyzw
			mulps    xmm6, [edx+0x20] // t5 zzzz * row2
			mov      edx, bone2
			// stall xmm4 (1)
			addps    xmm4, xmm3 // t4 + t2 xxxx
			movaps   xmm2, xmm0 // t6 xyzw
			unpcklps xmm0, xmm0 // t6 xxyy
			unpckhps xmm2, xmm2 // t6 zzww
			movaps   xmm1, xmm0 // t6 xxyy
			movlhps  xmm0, xmm0 // t6 xxxx
			movhlps  xmm1, xmm1 // t6 yyyy
			mulps    xmm0, [edx] // t6 xxxx * row0
			mulps    xmm1, [edx+0x10] // t6 yyyy * row1
			addps    xmm4, xmm5 // t4 + t5 xxxx yyyy
			movaps   xmm7, xmm2 // t6 zzww
			movlhps  xmm2, xmm2 // t6 zzzz
			movhlps  xmm7, xmm7 // t6 wwww
			mulps    xmm2, [edx+0x20] // t6 zzzz * row2
			addps    xmm1, xmm0 // t6 xxxx + yyyy

			// stall xmm4 (2)
			addps    xmm6, xmm1 // t4 + t5 + t6
			// stall xmm4 (3)
			shufps   xmm6, xmm6, 93h // wxyz
			// stall xmm4 (1)
			movss    xmm6, xmm7 // (w)xyz
			// stall xmm4 (1)
			shufps   xmm6, xmm6, 39h // xyzw
			// stall xmm4 (1)
			movaps [edi+0x30], xmm6 // [store] tangent 2
		}
#else
#error "implement me!"
#endif
