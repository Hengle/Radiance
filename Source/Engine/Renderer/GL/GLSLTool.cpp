...
//		ss << "#define SHADER_POSITION\r\n";
	...
	int numTexCoords = shader->MaterialSourceUsage(pass, Shader::kMaterialSource_TexCoord);
	if (numTexCoords > 0) {
		...
		for (int i = 0; i < r::kMaterialTextureSource_MaxIndices; ++i) {
			...
			int uvIndex = material.TCUVIndex((int)mapping.tcMods[i]);

			RAD_ASSERT(uvIndex > -1);
			if (uvIndex >= r::kMaxAttribArrays) {
				COut(C_Error) << "Material exceeds r::kMaxAttribArrays" << std::endl;
				return false;
			}

			ss << "#define TEXCOORD" << i << " tc" << uvIndex << "\r\n";
			++numTCMods;
		}
