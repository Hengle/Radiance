...
void Tokenizer::GetRest(String& sToken, bool crossline)
{
	sToken = "";
	...
	if (crossline)
	{
		// Gets all the remaining characters in the input stream.
		sToken = m_pOffset;
	}
	else
	{
		// Get all the remaining characters up to a newline.
		while(*m_pOffset != '\0')
		{
			if(*m_pOffset == '\n')
			{
...
bool Tokenizer::GetToken(String& sToken)
{
	if(m_bUnget)
	{
		m_bUnget = false;
