...
void WorldDraw::Counters::Clear() {
	...
	numLightPasses = 0;
	numLightPassLights = 0;
	numStencilLightPasses = 0;
	numBatches = 0;
	numTris = 0;
	numMaterials = 0;
}
...
void WorldDraw::VisMarkArea(
	...
	for (LightPtrSet::const_iterator it = area.lights.begin(); it != area.lights.end(); ++it) {
		...
		if ((light.m_visFrame != m_markFrame) &&
			...
			(light.intensityScale >= 0.01f) || 
			(light.shadowWeight < 0.f))) {
	...
	// mark world models.
	if (m_world->cvars->r_drawworld.value) {
	...
	// add entities.
	if (m_world->cvars->r_drawentities.value) {
	...
	if (m_world->cvars->r_drawoccupants.value) {
